...
<?php $__env->startSection('content'); ?>
    ...
    <script type="text/javascript"
            src="https://cdnjs.cloudflare.com/ajax/libs/fancybox/3.5.2/jquery.fancybox.min.js"></script>
                ...
                <div class="col-md-8">
                    ...
                    <div class="row box-icon text-center">
                        <div class="col-md-3">
                            <p>
                                ...
                                <?php endif; ?>
                                <?php if($count > 0): ?>
                                    <?php echo e($count); ?> مهمان
                                    <?php if($hostModel->count_guest - $count > 0): ?>
                                        + <?php echo e($hostModel->count_guest - $count); ?> نفر اضافه
                                    <?php endif; ?>
                                <?php else: ?>
                                    <?php echo e($hostModel->count_guest); ?> مهمان
                                <?php endif; ?>
                    ...
                    <div class="row">
                        ...
                        <div class="row section-comment">
                            ...
                            <?php $__currentLoopData = $commentModel; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $value): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <div class="row">
                    ...
                    <div id="onwe" class="col-md-4 dd form-reserve1">
                        <form class="form-reserve">
                            <div class="row">
                                <div class="col-md-12">
                                    <label class="form-label animation-color">هر شب از <?php echo e(GetMinPrice($hostModel->id)); ?> تومان</label>
                                </div>
                            </div>
        ...
        <script>
            ...
            <script>
                ...
                (function ($) {
                    $(window).scroll(function () {
                        ...
                        if ($heightScrolled < $defaultHeight) {
                            ...
                            $('#left-banner').removeClass("fixed2")
                            $('.form-reserve').removeClass("fixed2")
                            $('#sidebar-right1').removeClass("fixed2")
                            $('#sidebar-left1').removeClass("fixed1")
                        } else {
                            $('.top1').addClass("fixed")
                            $('#left-banner').addClass("fixed2")
                            $('.form-reserve').addClass("fixed2")
                            $('.form-reserve').removeClass("fixed31")
                            $('#sidebar-right1').addClass("fixed2")
                            $('#sidebar-left1').addClass("fixed1")
                        }
                ...
                (function ($) {
                    ...
                    setTimeout(function () {
                        footers = $(".footer");
                        position = footers.offset();
                        defaultHeight1 = position.top;
                        done = 1;
                    }, 500);

                    $(window).scroll(function () {
                        if (done == 1) {
                            var heightScrolled = $(document).scrollTop();
                            // console.log('heightScrolled: ' + heightScrolled + ' defaultHeight1: ' + defaultHeight1);
                            if ((heightScrolled + 850) >= defaultHeight1) {
                                $('.form-reserve').addClass("fixed31")
                                $('.form-reserve').removeClass("fixed2")
                                $('#left-banner').removeClass("fixed2")
                                $('#sidebar-right1').removeClass("fixed2")
                                $('#sidebar-left1').removeClass("fixed1")
                            } else {
                                $('.form-reserve').addClass("fixed34")
                            }
                        }
                    });
            ...
            <script language="javascript">
                ...
                $(document).ready(function () {
                    ...
                    alertSession('ورود به سیستم', 'نام کاربری یا رمز عبور اشتباه است.');
                    <?php elseif(session()->has('login_status')): ?>
                    $('#myModalRegister').modal('show');
                    alertSession('ورود به سیستم', 'کاربر غیر فعال می باشد.');
                    <?php elseif(session()->has('active_code')): ?>
                    $('#myModalRegister').modal('show');
                    alertSession('ثبت نام', 'کد تاییدیه برای شما ارسال شد، لطفا منتظر بمانید.');
                    <?php elseif(session()->has('success_register')): ?>
                    $('#myModalRegister').modal('show');
                    alertSession('ثبت نام', 'ثبت نام با موفقیت انجام شد.');
                    <?php elseif(session()->has('confirm_password')): ?>
                    $('#myModalRegister').modal('show');
                    alertSession('ثبت نام', 'تاییدیه رمز ورود اشتباه می باشد.');
                    <?php elseif(session()->has('error_register')): ?>
                    $('#myModalRegister').modal('show');
                    alertSession('ثبت نام', 'خطا در ثبت نام، لطفا مجددا تلاش نمایید.');
                    <?php endif; ?>
                    ...
                    $('#BtnReserveHost, .btn-reserve').click(function () {
                        ...
                        if (date_from == '' || date_to == "") {
                            AlertMessage('خطا در ارسال اطلاعات', 'لطفا تاریخ شروع و پایان را تعیین کنید', 'warning');
                            return false;
                        }
                        var host_id = <?php echo e($hostModel->id); ?>;
                        var count_guest = $('#CountGuest').val();
                        $('.form-reserve').css('opacity', '0.6');
                        $('.display-block').css('display', 'block');
                        $('#SpanLoading').html('<img style="width: 25px;" src="<?php echo e(asset('backend/img/img_loading/orange_circles.gif')); ?>" />');
                        ...
                        $.ajax({
                            url: '<?php echo e(route('ReserveHostByDate')); ?>',
                            type: 'post',
                            data: {
                                date_from: date_from,
                                date_to: date_to,
                ...
                // Alert for check session
                function alertSession(title, message) {
                    $.Toast("<p>" + title + "</p>", "<p>" + message + "</p>", "warning", {
                        has_icon: true,
                        has_close_btn: true,
                        stack: true,
                        fullscreen: false,
                ...
                $(document).on('click', '.btn-add', function () {
                    ...
                    if (value == <?php echo e($hostModel->count_guest); ?>) {
                        return false;
                    }

...
<?php $__env->startSection('content'); ?>
    <div class="row back-url">
        <div class="col-md-12">
            <a class="" href="<?php echo e(route('AdminDashboard')); ?>">صفحه اصلی</a><span class="now-url"> / نوع خانه</span>
        </div>
    </div>
    <div class="row">
        <div class="col-md-12">
        ...
        <div class="col-md-12">
            <div class="panel panel-primary">
                <div class="panel-heading">
                    <h3 class="panel-title">نوع خانه</h3>
                </div>
                <div class="panel-body">
                    <form role="form" action="<?php echo e(route('StoreHomeType')); ?>" method="post" enctype="multipart/form-data" id="FormCreateHomeType">
                        <?php echo e(csrf_field()); ?>

                        <div class="row">
                            <div class="col-md-3">
                                <div class="form-group">
                                    <label for="InputTitleHomeType">عنوان</label><span style="font-size: 18px;" class="text-danger">*</span>
                                    <input type="text" name="name" value="<?php echo e(old('name')); ?>" class="form-control primary" dir="rtl" id="InputTitleHomeType" placeholder="عنوان را وارد کنید" autofocus>
                                </div>
                    ...
                    <table cellpadding="0" cellspacing="0" border="0"
                        ...
                        <?php $__currentLoopData = $homeTypeModel; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $value): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <tr class="1">
                                ...
                                <td>
                                    <label id="btnActiveHomeType<?php echo e($key+1); ?>" onclick="ajaxActiveHomeType('<?php echo e($value->id); ?>','<?php echo e($key+1); ?>');" class="<?php if($value->active == 1): ?> btn btn-default btn-block <?php else: ?> btn btn-default btn-block <?php endif; ?>">
...
<?php $__env->startSection('script'); ?>
    ...
    <script>
        ...
        function ajaxEditName(id) {
            ...
            $.ajax({
                url:"<?php echo e(url('edit/home-type').'/'); ?>"+id,
                method:"get",
        ...
        function ajaxActiveHomeType(id,idInput) {
            $.ajax({
                url:"<?php echo e(url('active/home-type').'/'); ?>"+id,
                method:"get",
            }).done(function(returnData){
                // console.log(returnData);
                ...
                {
                    // console.log('active');
                    $("#btnActiveHomeType"+idInput).addClass('btn-default');
                    $("#btnActiveHomeType"+idInput).removeClass('btn-default');
                    $("#btnActiveHomeType"+idInput).css("transition", "0.5s");
                    $("#btnActiveHomeType"+idInput).text('');
                    $("#btnActiveHomeType"+idInput).append('<i class="fa fa-check text-success"></i>');
                }
                else if(returnData == 'deactive')
                {
                    ...
                    $("#btnActiveHomeType"+idInput).css("transition", "0.5s");
                    $("#btnActiveHomeType"+idInput).text('');
                    $("#btnActiveHomeType"+idInput).append('<i class="fa fa-plus text-danger" style="transform: rotate(45deg)"></i>');
                }
            });
        }

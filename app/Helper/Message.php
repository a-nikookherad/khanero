...
function RandomName()
{
    $array=array('q','w','e','r','t','y','u','i','o','p','a','s','d','f','g','h','j','k','l','z','x','c','v','b','n','m',
...
function PrintMessage($Message,$AlertType='alert-danger')
{
...
function GetMinPrice($host_id) {
    $hostModel = Host::where('id', $host_id)->first();
    if(!empty($hostModel)) {
        $priceDayModel = PriceDay::where('host_id', $host_id)->min('price');
        return $priceDayModel;
    }
}
...
function GetProvince() {
    $province = Province::where('active', 1)->get();
    return $province;
}

use App\Modules\City\Model\Township;
function GetTownship($id) {
    $township = Township::where('province_id', $id)->get();
    return $township;
}

use App\Modules\Possibilities\Model\Option;
function GetOption() {
    $option = Option::where('active', 1)->get();
    return $option;
}

use App\Modules\Rules\Model\Rule;
function GetRule() {
    $rule = Rule::where('active', 1)->get();
    return $rule;
}

function PersianDate($date)
{
    return \Morilog\Jalali\Facades\jDate::forge($date,'Asia/Tehran')->format('Y/m/d');
}

use App\Modules\Reserve\Model\Reserve;
function GetCountReserve($id)
{
    return Reserve::where('host_id', $id)->count();
}

function GetReserveByCode($code) {
    return Reserve::where('group_code', $code)->get();
}
...
function DocumentsHost()
{
    return array(
        ...
        [
            'id' => '2',
            'name' => 'تصویر قرارداد اجاره یا سند'
        ],
        [
            'id' => '3',
            'name' => 'پروانه کسب'
...
function GetTypeDocument($type)
{
    switch ($type) {
        ...
        case '2':
            return 'تصویر قرارداد اجاره یا سند';
        case '3':
            return 'پروانه کسب';
        case '4':
            return 'قبض آب';
...
{
    $start_date=\Morilog\Jalali\Facades\jDateTime::ConvertToGeorgian($date,date("00:00:00"));
    $dt = \Carbon\Carbon::parse($start_date);

    return $dt->dayOfWeek;
}

use App\Modules\Week\Model\Week;
function GetNameDayOfWeek($id)
{
    $weekModel = Week::where('id', $id)->first();
    return $weekModel->day;
}


//generate code Introduction
function GenerateCodeIntroduction($id)
{
    $result = $id*3;
    $result = $result*30458948065230;
    return $result;
}


//generate id Introduction
function GenerateIdIntroduction($code)
{
    $result = $code/30458948065230;
    $result = $result/3;
    return $result;
}
...
/*	F	*/
function bdate($format,$timestamp='',$none='',$time_zone='Asia/Tehran',$tr_num='fa'){
    ...
    for($i=0; $i<$sl; $i++){
        ...
        switch($sub){
            ...
            case'D':
                $out.=bdate_words(array('kh'=>$date[7]),' ');
                break;

            case'f':
                $out.=bdate_words(array('ff'=>$j_m),' ');
                break;

            case'F':
                $out.=bdate_words(array('mm'=>$j_m),' ');
                break;
            ...
            case'l':
                $out.=bdate_words(array('rh'=>$date[7]),' ');
                break;
            ...
            case'r':
                $key=bdate_words(array('rh'=>$date[7],'mm'=>$j_m));
            ...
            case'v':
                $out.=bdate_words(array('ss'=>($j_y%100)),' ');
                break;

            case'V':
                $out.=bdate_words(array('ss'=>$j_y),' ');
                break;
...
/*	F	*/
function bstrftime($format,$timestamp='',$none='',$time_zone='Asia/Tehran',$tr_num='fa'){
    ...
    for($i=0; $i<$sl; $i++){
        ...
        switch($sub){
            ...
            /* Day */
            case'a':
                $out.=bdate_words(array('kh'=>$date[6]),' ');
                break;

            case'A':
                $out.=bdate_words(array('rh'=>$date[6]),' ');
                break;
            ...
            case'c':
                $key=bdate_words(array('rh'=>$date[6],'mm'=>$j_m));
...
/*	F	*/
function bmktime($h='',$m='',$s='',$jm='',$jd='',$jy='',$none='',$timezone='Asia/Tehran'){
        ...
        if($m===''){
            ...
            if($s===''){
                ...
                if($jm===''){
                    ...
                    $jdate=explode('_',bdate('Y_j','','',$timezone,'en'));
                    if($jd===''){
...
/*	F	*/
function bgetdate($timestamp='',$none='',$timezone='Asia/Tehran',$tn='en'){
    $ts=($timestamp==='')?time():tr_num($timestamp);
    $jdate=explode('_',bdate('F_G_i_j_l_n_s_w_Y_z',$ts,'',$timezone,$tn));
    return array(
...
/*	F	*/
function bdate_words($array,$mod=''){

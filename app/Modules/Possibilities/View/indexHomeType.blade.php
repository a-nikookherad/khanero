...
@section('content')
    <div class="row back-url">
        <div class="col-md-12">
            <a class="" href="{{ route('AdminDashboard') }}">صفحه اصلی</a><span class="now-url"> / نوع خانه</span>
        </div>
    </div>
    <div class="row">
        <div class="col-md-12">
        ...
        <div class="col-md-12">
            <div class="panel panel-primary">
                ...
                <div class="panel-body">
                    <form role="form" action="{{ route('StoreHomeType') }}" method="post" enctype="multipart/form-data" id="FormCreateHomeType">
                        {{ csrf_field() }}
                        <div class="row">
                            <div class="col-md-3">
                                <div class="form-group">
                                    <label for="InputTitleHomeType">عنوان</label><span style="font-size: 18px;" class="text-danger">*</span>
                                    <input type="text" name="name" value="{{ old('name') }}" class="form-control primary" dir="rtl" id="InputTitleHomeType" placeholder="عنوان را وارد کنید" autofocus>
                                </div>
                                @if($errors->has('name'))
                    ...
                    <table cellpadding="0" cellspacing="0" border="0"
                        ...
                        <tbody>
                        @foreach($homeTypeModel as $key => $value)
                            <tr class="1">
                                <td>
                                    {{ $key+1 }}
                                </td>
                                ...
                                <td>
                                    <label id="btnActiveHomeType{{$key+1}}" onclick="ajaxActiveHomeType('{{ $value->id }}','{{$key+1}}');" class="@if($value->active == 1) btn btn-default btn-block @else btn btn-default btn-block @endif">
